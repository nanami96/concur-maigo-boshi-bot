...
import { parseInitialSetupExcel } from "../src/flow/parseInitialSetupExcel";
...
describe("parseInitialSetupExcel 正常系", () => {
  it("01_基本設定にschema_version列が無くても正常に解析できる（Excel仕様は常に最新の1種類のみをサポートし、バージョン列は廃止した）", () => {
  ...
  it("schema_version列が残ったままの古いExcelでも、他の必須列さえ揃っていれば余分な列として無視して正常に解析できる", () => {
    const workbook = buildWorkbook(
      buildValidSheets({
        "01_基本設定": [["会社ID", "会社名", "schema_version"], ["test-co", "テスト会社", 1]],
      }),
    );
    const result = parseInitialSetupExcel(workbook);
  ...
  it("会社IDが空欄の場合は会社名から自動生成される", () => {
    const workbook = buildWorkbook(
      buildValidSheets({ "01_基本設定": [["会社ID", "会社名"], ["", "テスト会社2"]] }),
    );
    const result = parseInitialSetupExcel(workbook);
...
describe("parseInitialSetupExcel: sample-company実データテンプレートでの検証", () => {
  it("excel/templates/initial-setup-template.xlsx を読み込むと件数・flowが完全一致する", () => {
  ...
  it("07_Concurマッピングシートは廃止済み（経費タイプコード＝Concur EXP_KEYという設計により不要）", () => {
    ...
    expect(workbook.SheetNames).not.toContain("07_Concurマッピング");

    const result = parseInitialSetupExcel(workbook);

    expect(result.errors).toEqual([]);
    expect(result).not.toHaveProperty("concurExpenseTypeMappings");
    expect(result).not.toHaveProperty("hasConcurMappingSheet");
    expect(result.expenseTypes).toHaveLength(79);
  });
