...
export function runConfigChecks({ company, policies, expenseTypes, flow, concurExpenseTypeMappings }) {
  const flowResult = checkFlow(flow, expenseTypes);
  const masterDataResult = checkMasterData({ company, policies, expenseTypes, flow, concurExpenseTypeMappings });
