...
`sample-company` は新スキーマ（関係モデル）、`company-a` は旧スキーマを使用している。`scripts/generate-config.js` はブック内に `04_質問` シートが存在するかどうかで自動的にどちらの読み込み方式かを判定するため、担当者側で特別な指定は不要。

### 新スキーマ（sample-company）

| シート | 編集するタイミング |
| --- | --- |
| `04_質問` | 質問文や質問の表示順を追加・変更するとき（質問IDはQ001形式で末尾に追加） |
| `05_選択肢` | 選択肢の表示文言や次に質問する質問IDを追加・変更するとき（選択肢IDはO001形式で末尾に追加） |
| `06_判定ルール` | 質問ID・選択肢IDの組み合わせに対して、どの経費タイプへ案内するかを追加・変更するとき（同じ組み合わせに複数行を追加すると、React画面で「候補となる経費タイプ」として複数表示される） |
| `03_経費タイプ` | Concurで利用する経費タイプを追加・変更するとき |
| `02_ポリシー` | ポリシーを追加・変更するとき |
| `01_基本設定` | 会社ID、会社名など会社単位の基本設定を変更するとき |
...
一部のシートでは2行目からデータを扱う場合があります（新スキーマの全シートは2行目から実データ）。既存のシート構造に合わせて、途中に行を挿入せず、既存データの末尾へ追加してください。
